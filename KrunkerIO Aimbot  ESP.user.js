...
function inject() {
	...
	const old = WeakMap.prototype.get;

	WeakMap.prototype.get = function ( object ) {
...
function animate() {
	...
	if ( ! injected ) {
		...
		if ( document.querySelector( '#initLoader' ).style.display === 'none' ) {
...
<div class="dialog">${shouldShowAd ? `<big>Loading ad...</big>` : `<div class="close" onclick="this.parentNode.style.display='none';"></div>
	...
	<div style="display: grid; grid-template-columns: 1fr 1fr; grid-gap: 5px;">
		...
		<div class="btn" onclick="window.open('https://www.instagram.com/acexduke/', '_blank')">Instagram</div>
...
if ( shouldShowAd ) {
	...
	window.location.href = '?ref=' + new TextEncoder().encode( url.href ).toString();
